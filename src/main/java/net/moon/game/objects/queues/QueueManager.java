...
public class QueueManager {
    ...
    public void run() {
        this.queues.values().forEach(kitQueueMap -> kitQueueMap.values().forEach(Queue::run));
    }

    public List<Queue> getByType(final QueueType type) {
        return this.queues.get(type).values().stream().toList();
    }
}
